...
In the case of independent and identically distributed $\epsilon_p(i)$'s, \citet{barndorff1963limit} and \citet{resnick1973almost} obtained necessary and sufficient conditions for the \emph{almost sure stability} of maxima, where the convergence in \eqref{eq:RS-condition} holds almost surely. \stilian{See also \cite{klass:1984} for further 
sharp results on almost sure stability.  Here, we will only need the weaker notion in \eqref{eq:RS-condition} 
but extend our inquiry to the case of dependent $\epsilon_p(i)$'s.}{\fbox{The ref suggested by a reviewer -- Resnick?}}
...
\stilian{ 
\begin{remark} \label{rem:iid-max} Part (1) of Proposition \ref{prop:rapid-varying-tails} is equivalent to 
\begin{equation}\label{e:rem:iid-max}
  \P\left[\frac{1}{u_p} M_p >  1+\delta_p \right] \longrightarrow 0,\ \ \mbox{ as }p\to\infty,
\end{equation}
for some positive sequence $\delta_p\to 0$.  Notice on the other hand that if $M_p^*$ is the maximum of $p$ iid variables with distribution $F$,
...
maxima $M_p^*$.  This somewhat surprising fact is true regardless of the dependence structure of ${\cal E}$ and is solely a 
consequence of the rapid variation of $F$.
\end{remark}
}{\fbox{The above remark addresses a question of a reviewer}}
...
\stilian{More details on these alternative classes of models can be found in the Supplement.}
{\fbox{Move these to the online Supplement?} 
