\stilian{R2, Typo 4}{I suggest we move all exercises to the online Supplement.  The supplement can have several chapters, the first of which could be 
"A crash course on extremes" or ``Selected background from EVT".  It could address some comments on the reviewers that the practitioners could need 
a lot of ``hand holding".  Similarly, in the Supplement we can put other ``basic" things we would like to have like ``Standard" derivations on the Sidak, BH procedure
the definition of Tukey's Higher Criticism statistics and the results from Empirical processes that are used to study it...} \fbox{ TO DO: Verify and add}
...
\stilian{R2, Typo 5}{ \fbox{TO DO} {\em Hint:} \fbox{ Give a good hint. } }
