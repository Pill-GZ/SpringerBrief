...
\stilian{\fbox{SUGGESTION}}{ What do you think about making the Bayesian stuff a Chapter?  I think it stands well on its own... plus it is somewhat different from the ``non-Bayesian'' setup of the previous support recovery results...}

\stilian{
In studying support recovery problems \citep[e.g.,][]{arias2017distribution}, often the attention is restricted to the class of
thresholding procedures.  This is justified by arguing that such procedures are the ``reasonable'' choice for estimating the support set.
...
We show in this section that, in many cases, the finite-sample Bayes optimal procedures do indeed belong to the class of thresholding procedures.  
Perhaps surprisingly, however, we also show that for general error models, the thresholding procedures are sub-optimal, 
even when the observations are independent.  Roughly speaking, in the additive error 
models \eqref{eq:model-additive},  this distinction depends on whether or not the error density is log-concave 
(Proposition \ref{prop:log-concave} and Corollary \ref{cor:log-convex}).}{\fbox{edited, agree?}}

We shall identify the optimal procedure for support recovery problems under a Bayesian setting with general distributional assumptions 
(including but not limited to additive models \eqref{eq:model-additive}). 
Specifically, consider an ordered set $P = (i_1,\ldots,i_s)$ of $s$ distinct indices in $\{1,\ldots,p\}$, which will be referred to 
as the {\em ordered support} of the signal.  Consider also $s$ (not necessarily equal) signal densities 
$f_{1}, \ldots, f_{s}$ as well as an error density $f_0$. We shall assume that the observations with indices $i_j$ 
are drawn independently from $f_j$:
\begin{equation} \label{eq:signal-distributions-ordered}
x(i_j) \sim f_j, \quad j=1,\ldots,s,
\end{equation}
while the remaining $(p-s)$ observations have common density $f_0$, i.e., $x(i)\sim f_0$ for $i\not\in S$. 
We further assume that the observations $x$ are mutually independent.


We adopt now a Bayesian framework to measure statistical risks and assume that the underlying (unknown) support is {\em random}. Namely,
...
Consequently, the unordered support $S=\{i_1,\ldots,i_s\}$ is distributed uniformly over the collection of all sets of size $s$, with 
the unordered uniform distribution $\pi^{\text{u}}$. That is, for all $S\in\mathcal{S}:=\left\{S\subseteq\{1,\ldots,p\};|S|=s\right\}$, we have 
...
\stilian{(The $x(i)$'s are conditionally independent, given the ordered support $P$.)}{\fbox{added.  Agree?}}
...
The relationship between the above two notions is summarized next.
