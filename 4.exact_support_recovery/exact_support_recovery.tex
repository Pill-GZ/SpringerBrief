...
%The signal sizes now depends on the shape of the assumed error distributions $\mathrm{AGG}(\nu)$ through the parameter $\nu$.
 ...
 \stilian{\begin{remark}Without much effort, one can relax the above parameterization of the support size in \eqref{eq:sparsity-parametrized} to $|S_p| \sim p^{1-\beta}$ or in fact, to the 
