...
\begin{align} \label{eq:minimax-signal-config-over}
    \Theta_p^+(\beta, \underline{r}) &= \{\mu\in\mathbb{R}^p:\;\text{there exists a set }S_p\subseteq\{1,\ldots,p\}\;\text{ such that }|S_p|\le s_p^\dagger, \nonumber \\
    &\quad\quad\mu(i)\ge (\nu\underline{r}\log{p})^{1/\nu}\;\text{for all }i\in S_p,\;\text{and }\mu(i)=0\;\text{for all }i\not\in S_p\},
\end{align}
where $s_p^\dagger$ is as in \eqref{eq:sparsity-parametrized}.
...
Let the errors have common marginal distribution $F\in \text{AGG}(\nu)$ with $\nu>0$.  Let $\widehat{S}_p$ be the Bonferroni's procedure \eqref{eq:Bonferroni-procedure} with vanishing FWER $\alpha = \alpha(p) \to 0$, such that %\fbox{slower than any polynomial}
...
\begin{align}
    \P\Big[\widehat{S} \supseteq S\Big]
      ...
      &\ge 1 - \ell(p)p^{1-\beta} \times \overline{F}_-(\text{A}), \label{eq:sufficient-proof-eq2}
...
Notice that \eqref{eq:sufficient-proof-eq2} follows from the union bound and the assumption that ${|S_p|}\le s_p^\dagger= \ell(p)p^{1-\beta}$ in \eqref{eq:minimax-signal-config-over}. 
...
\begin{equation} \label{eq:exact-supporot-recovery-FWNR}
    \inf_{\mu\in\Theta_p^+(\beta, \underline{r})} \P[\widehat{S}_p \supseteq S_p] \ge 1 - \ell(p)p^{1-\beta} \times \overline{F}_-(\text{A}).
\end{equation}

We first show that the right-hand-side of \eqref{eq:exact-supporot-recovery-FWNR} converges to $1$ when $\beta=1$. 
Indeed, since $F\in {\rm AGG}(\mu)$, we have, for sufficiently large $p$,
$$
\overline{F}_{-}({\rm A}) \le \overline{F}_-(c(\nu \log(p))^{1/\nu}) = {\cal O}( p^{-c'} ),
$$
for some $c>c'>0$.
On the other hand, the celebrated Potter bounds for slowly-varying functions \citep[see, e.g.,][]{bingham:goldie:teugels:1987} entail $\ell(p) = o(p^{c'})$, for every $c'>0$ and hence $\ell(p) \overline{F}_{-}({\rm A}) \to 0$, as $p\to\infty$.

Let now $\beta\in (0,1)$ and $u_p^- :=  F_-^{\leftarrow}(1-1/p)$. 
The fact that $p\overline{F}_-(u_p^-) \le 1$, implies
\begin{equation}
    s_p^\dagger \times \overline{F}_-(\text{A}) \le 
...
Notice that by assumption, the $-\epsilon(i)$'s are also AGG$(\nu)$ distributed, and by Proposition \ref{prop:quantile}, $u_p^{-}:= F_-^{\leftarrow}(1-1/p) \sim (\nu \log(p))^{1/\nu}$, as $p\to\infty$.
Therefore, we have
\begin{equation} \label{eq:sufficient-proof-eq4}
    u_{s_p^\dagger}^- \equiv u_{\ell(p)p^{1-\beta}}^-\sim \left(\nu(1-\beta)\log{p}\right)^{1/\nu},
\end{equation}
where we used the fact that $\log(\ell(p)) = o(\log(p))$. Hence,
\begin{equation*}
...
\begin{align} \label{eq:minimax-signal-config-under}
    ...
    |S_p| = \lfloor s_p^\dagger \rfloor, \nonumber \\
    &\quad\quad0<\mu(i)\le(\nu\overline{r}\log{p})^{1/\nu}\;\text{for all }i\in S_p,\;\text{and }\mu(i)=0\;\text{for all }i\not\in S_p\},
\end{align}
where $s_p^\dagger= \ell(p) p^{1-\beta}$ is as in \eqref{eq:sparsity-parametrized}.
...
$$
S_p^* = \argmax_{S\subseteq\{1,\ldots,p\}:|S| = \lfloor s_p^\dagger \rfloor} \P\left[{\max_{i\in S^c}\epsilon(i)} < {\overline{\Delta} + \min_{i\in S}\epsilon(i)}\right],
$$
where $s_p^\dagger = \ell(p)p^{1-\beta}$ is the size of the true support set, and 
ties are broken lexicographically if multiple maximizers exist.
Then,
...
Notice that the second relation in \eqref{eq:classification-possible-dependent-proof-3} holds by URS for any $\beta\in(0,1)$.
When $\beta=1$, the relation holds because $\{{m_{S^*}}/{u_{|S^*|}}\}$ is tight, while $0\le u_{|S^*|}/u_p \le u_{\ell(p)}/u_p \to 0$ since $\ell(p) = o(p)$ by the Potter bounds for slowly varying functions \citep[see, e.g.,][]{bingham:goldie:teugels:1987}.
...
$\mathcal{S} = \left\{S\subseteq\{1,\ldots,p\}; |S| = \lfloor \ell (p) p^{1-\beta}\rfloor \right\}$, we have
\begin{equation}
