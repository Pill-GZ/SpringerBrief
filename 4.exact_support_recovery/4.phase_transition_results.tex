...
\end{align}
\stilian{where $s_p^\dagger$ is as in \eqref{eq:sparsity-parametrized-new}.}
...
that  \stilian{${|S_p|}\le s_p^\dagger= \ell(p)p^{1-\beta}$ in \eqref{eq:minimax-signal-config-over}. }{${|S_p|}\le \lfloor p^{1-\beta}\rfloor$}. 
...
converges to $1$. \stilian{ Indeed, since $F\in {\rm AGG}(\mu)$, we have that 
$$
\overline{F}_{-}({\rm A}) \le \overline{F}_-(c(\nu \log(p))^{1/\nu}) = {\cal O}( p^{-c'} ),
$$
for some $c>c'>0$.  On the other hand, the celebrated Potter bounds for slowly-varying functions \citep[see, e.g.,][]{bingham:goldie:teugels:1987}
entail $\ell(p) = o(p^{c'})$, for every $c'>0$ and
hence $\ell(p) \overline{F}_{-}({\rm A}) \to 0$, as $p\to\infty$.}{ since $\text{A}\to+\infty$.}
...
\end{equation}
\stilian{where we used the fact that $\log(\ell(p)) = o(\log(p))$. Hence,}{and} 
\begin{equation*}
...
\end{align}
\stilian{where $s_p^\dagger= \ell(p) p^{1-\beta}$ is as in \eqref{eq:sparsity-parametrized-new}.}
...
$$
\stilian{where $s_p^\dagger = \ell(p)p^{1-\beta}$ is the size of the true support set and}{}
...
Notice that the second relation in \eqref{eq:classification-possible-dependent-proof-3} holds by URS for any $\beta\in(0,1)$; when $\beta=1$, the relation holds because \stilian{$\{{m_{S^*}}/{u_{|S^*|}}\}$ is tight while $0\le u_{|S^*|}/u_p \le u_{\ell(p)}/u_p \to 0$ since $\ell(p) = o(p)$ 
by the Potter bounds for slowly varying functions \citep[see, e.g.,][]{bingham:goldie:teugels:1987}. }{${u_{|S^*|}}/{u_p}$  vanishes while $\{{m_{S^*}}/{u_{|S^*|}}\}$ is tight.}
