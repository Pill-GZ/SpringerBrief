...
Let the errors have common marginal distribution $F\in \text{AGG}(\nu)$ with $\nu>0$.  \stilian{If $\beta=1$, assume in addition that the
slowly varying function in \eqref{eq:sparsity-parametrized-new} is such that
\begin{equation}\label{e:new-beta=1-condition}
\ell(p) = o\Big(1/ F(- c\log(p)^{1/\nu})\Big),\ \ \mbox{ for all $c>0$.}
\end{equation}}{}
...
\end{equation}
\end{theorem}
\stilian{\begin{remark} The seemingly technical condition \eqref{e:new-beta=1-condition} is rather mild.
It is trivially satisfied if $\ell(p) \sim {\rm const}$.  This condition is only needed in the extremely sparse regime $\beta=1$.
\end{remark}}{}
...
converges to $1$, \stilian{in view of \eqref{e:new-beta=1-condition} applied with $c \log(p)^{1/\nu}:= A$.}{ since $\text{A}\to+\infty$.}
...
Notice that the second relation in \eqref{eq:classification-possible-dependent-proof-3} holds by URS for any $\beta\in(0,1)$; when $\beta=1$, the relation holds because \stilian{$\{{m_{S^*}}/{u_{|S^*|}}\}$ is tight while $0\le u_{|S^*|}/u_p \le u_{\ell(p)}/u_p \to 0$ since $\ell(p) = o(p)$. }{${u_{|S^*|}}/{u_p}$  vanishes while $\{{m_{S^*}}/{u_{|S^*|}}\}$ is tight.}
