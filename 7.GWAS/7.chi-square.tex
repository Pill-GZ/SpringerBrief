...
\end{equation}
where $S_p:= \{ i\, :\, \lambda(i) >0 \}$ is the {\em signal support} set and  $\beta$ parametrizes the problem {\em sparsity}. \stilian{More general
parameterizations of the support size are possible as in \eqref{eq:sparsity-parametrized-new}. 
Here, however, for simplicity, we drop the slowly varying term $\ell(\cdot)$. } {}
