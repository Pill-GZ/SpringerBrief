...
fact finite-sample optimal for certain classes of models.  These optimality results allow us to establish 
...
\ref{exmp:suboptimal-data-thresholding}).   In this case, the likelihood ratio
thresholding rather than data thresholding can lead to optimal support estimators
...
\stilian{Continuing this way, we can successively improve the likelihood of every estimator until we arrive at an oracle thresholding estimator, proving the desired optimality.  Note that with the same argument, any two oracle 
thresholding estimators have the same likelihood.}{
\fbox{replacing} This implies that any estimator that is not $\widehat{S}^*$ may be improved, and the optimality follows.}
\end{proof}
...
%
\stilian{In this section, we demonstrate that thresholding the \emph{likelihood ratio} rather than signal values
...
\stilian{NOTE}{This change \& definition
 was motivated by the annoying fact that we could have ties among the likelihood ratios.  Also, to my understanding we
 must LIMIT OURSELVES to estimators of size $s$, so we should limit $|\widehat{S}_{{\rm LRT}}|$ to $s$. \fbox{CORRECT?} I am SORRY if I made a MESS.}
...
\begin{equation*}
    ...
    = \log{L(j)} - \log{L(j')} > 0
\end{equation*}
\stilian{This shows that $\widehat S$ cannot be Bayes optimal unless it is a likelihood thresholding estimator.
...
}{\fbox{Please check.} See also the change in the oracle thresholding argument.}
\end{proof}
...
\begin{remark}[An open problem] As we have seen, the thresholding estimators are no longer optimal 
...
Consider the model \eqref{eq:model-additive} with independent errors. Theorem \ref{thm:necessary-strengthened} 
and Corollary \ref{cor:minimax-over-dependence} provide a nearly complete picture of the difficulty in the exact support 
recovery problem, in the regime when the thresholding estimators are optimal. Specifically, in such cases the signal classification boundary is universal.


 Proposition \ref{prop:likelihood-ratio-thresholding}, and indeed, Example \ref{exmp:suboptimal-data-thresholding} demonstrate that thresholding procedures are in fact \emph{sub-optimal} for $\text{AGG}(\nu)$ models with $\nu<1$.
