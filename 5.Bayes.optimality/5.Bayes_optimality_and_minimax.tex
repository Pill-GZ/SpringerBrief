...
For the second statement \eqref{eq:point-wise-minimax-below}, since $\underline{r}<f_{\mathrm{E}}(\beta)$, we can pick a sequence $\mu^*\in\Theta_p^+(\beta, \underline{r})$ such that $|S_p| = \lfloor \ell(p) p^{1-\beta} \rfloor$, with signals having the same signal size $\mu(i)=(2r\log{p})^{1/\nu}$ for all $i\in S_p$, where $\underline{r}<{r}<f_{\mathrm{E}}(\beta)$.
...
\begin{align*}
    \Theta_p^* &=
    \{\mu\in\mathbb{R}^d:\;|S_p|=\lfloor \ell(p)p^{1-\beta} \rfloor,\;\mu(i)=0\;\text{for all }i\not\in S, \;\text{and}\\
