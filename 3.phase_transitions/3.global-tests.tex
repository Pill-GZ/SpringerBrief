...
without knowledge of the sparsity and signal sizes. It is also known that the max-statistic \eqref{eq:max-statistic} is only efficient when $r>(1+\sqrt{1-\beta})^2$, and is therefore sub-optimal for denser signals where $1/2\le\beta\le 3/4$; see \cite{cai2011optimal}.  \stilian{This said,  the very recent contribution of \cite{li2020optimality} investigates a more realistic setting where the non-null signal is drawn from a general distribution with tails no lighter than the Gaussian additive error.  In this, much more practical regime, they show that the max statistic in fact achieves the optimal detection boundary for the entire 
``sparsity'' regime $1/2<\beta<1$.  This surprising new development shows that the high-dimensional testing and inference is an active 
area of research.}{}
